...
import { MessageByType, Messages, MessageTypeEnum } from '../../services/Messaging';
...
export class Iframe {
     ...
     */
    private subscribe<T extends MessageTypeEnum>(messageType: T, handler: (message: MessageByType<T>) => void): void {
        const contentWindow = this.element.contentWindow;
        if (!contentWindow) {
            throw new Error('Iframe contentWindow is not available. Make sure the iframe is mounted and loaded.');
        }
